defmodule OperateTreeNode do
  ...
  def createTreeNode(nums) do
    createSubTreeNode(nums, 0, 0)
  end

  @spec createSubTreeNode(nums :: [String.t], depth :: Integer, pos :: Integer) :: TreeNode.t | nil
  def createSubTreeNode(nums, depth, pos) do
    cond do
      pos >= Enum.count(nums) ->
        nil
      Enum.count(nums) <= 0 ->
      ...
      true ->
        ...
        if (cur_pos + pos) > (Enum.count(nums) - 1) do
          ...
          if Enum.at(nums, cur_pos + pos) == "null" do
            ...
            %TreeNode{val: String.to_integer(Enum.at(nums, cur_pos + pos)), left: createSubTreeNode(nums, depth + 1, 2*pos), right: createSubTreeNode(nums, depth + 1, 2*pos + 1)}
          end
        end
    end
  end
