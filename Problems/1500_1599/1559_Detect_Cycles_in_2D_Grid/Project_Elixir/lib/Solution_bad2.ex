defmodule Solution_bad2 do
  ...
  def bfs(m_grid, m, n, i, j, x, y, visited) do
    visited = Map.put(visited, i, Map.put(visited[i], j, true))
    que = :gb_sets.new()
        ...
        {res, _cnt, que, visited} =
          Enum.reduce_while([{0, 1}, {1, 0}, {0, -1}, {-1, 0}], {false, cnt, que, visited}, fn {di, dj}, {_res, cnt, que, visited} ->
            ...
            cond do
              ...
              true ->
                {:cont, {false, cnt + 1, :gb_sets.add_element({cnt + 1, ni, nj, cur_i, cur_j}, que), Map.put(visited, ni, Map.put(visited[ni], nj, true))}}
            end
        end)
        if res do
