# coding: utf-8

import collections
import os
import sys
import time
import math
from typing import List, Dict, Tuple

class Solution:
    def countStudents(self, students: List[int], sandwiches: List[int]) -> int:
        # 36ms
        cnt = collections.Counter(students)
        n, k = len(students), 0
        while k < n and cnt[sandwiches[k]]:
            cnt[sandwiches[k]] -= 1
            k += 1
        return n - k

    def countStudents2(self, students: List[int], sandwiches: List[int]) -> int:
        # 36ms
        cnt = 0
        while len(students) > 0:
            if students[0] == sandwiches[0]:
                students = students[1:]
                sandwiches = sandwiches[1:]
                cnt = 0
            else:
                temp = students[0]
                students = students[1:]
                students.append(temp)
                cnt += 1
                if cnt == len(students):
                    break
        return len(students)


def main():
    argv = sys.argv
    argc = len(argv)

    if argc < 2:
        print("Usage: python {0} <testdata.txt>".format(argv[0]))
        exit(0)

    if not os.path.exists(argv[1]):
        print("{0} not found...".format(argv[1]))
        exit(0)

    testDataFile = open(argv[1], "r")
    lines = testDataFile.readlines()

    for temp in lines:
        temp = temp.strip()
        if temp == "":
            continue
        print("args = {0}".format(temp))
        loop_main(temp)
    #   print("Hit Return to continue...")
    #   input()

def loop_main(temp):
    flds = temp.replace("[[","").replace("]]","").replace("\"","").replace(" ","").rstrip().split("],[")

    students   = [int(n) for n in flds[0].split(",")]
    sandwiches = [int(n) for n in flds[1].split(",")]
    print("students   = {0}".format(students))
    print("sandwiches = {0}".format(sandwiches))

    sl = Solution()

    time0 = time.time()

    result = sl.countStudents(students, sandwiches)

    time1 = time.time()

    print("result = {0:d}".format(result))
    print("Execute time ... : {0:f}[s]\n".format(time1 - time0))

if __name__ == "__main__":
    main()
